...
def format_tespit(path: Path) -> str:
    ...
    wb = _xlsx_ac(path)
    ws = wb.active
    rows = list(ws.iter_rows(values_only=True, min_row=1, max_row=6))
...
def _xlsx_ac(path: Path) -> openpyxl.Workbook:
    """xls/xlsx fark etmeksizin openpyxl ile açar."""
    try:
        return load_workbook(path, data_only=True)
    except Exception:
        # .xls uzantılıysa kopyala
...
def vnk_oku(path: Path, log) -> pd.DataFrame:
    ...
    for i, c in enumerate(df.columns):
        ...
        elif "Tarih"      in s:  rename[c] = "Tarih"
        ...
        elif "Bor" in s and len(s) < 8:              rename[c] = "Borç"
        elif "Alacak" in s:                          rename[c] = "Alacak"
        elif "Bakiye" in s:                          rename[c] = "Bakiye"
        elif i in fallback:                          rename[c] = fallback[i]
...
def _sablon_df(**kw) -> pd.DataFrame:
    ...
    out["Borç (DR)\nDebit"]                                 = kw["borc"].values
    out["Alacak (CR)\nCredit"]                              = kw["alacak"].values
    out["Bakiye\nBalance"]                                  = kw["bakiye"].values
    out["Döviz Kuru\nExchange Rate"]                        = DOVIZ_KURU
    out["Dövizli Borç\nFCY Debit"]                         = kw["borc"].values
    out["Dövizli Alacak\nFCY Credit"]                      = kw["alacak"].values
    out["Dövizli Bakiye\nFCY Balance"]                      = kw["bakiye"].values
    out["Maliyet Merkezi\nCost Center"]                     = ""
